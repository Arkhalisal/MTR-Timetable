...
async function subline(subline) {
  const sublines = allLine[subline].sta.map((x) => x.code);
  console.log(sublines);
  for (const station of sublines) {
    take(subline, station);
    await delay(100);
  }
}
...
// let upData = []; // make an array to store up data
// let downData = []; // make an array to store down data
// let upDownData = data1.data[`${line}-${sta}`];
// // up and down data is an array !!!
// // push the first element of the fetched up array into our storage
// upData.push(upDownData.UP[0]);
// // push the first element of the fetched down array into our storage
// downData.push(upDownData.DOWN[0]);
// console.log(upData);
